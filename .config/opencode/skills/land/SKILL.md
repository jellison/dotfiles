...
On failure: **attempt to auto-fix** (use the `systematic-debugging` skill), then
**re-verify**. Bound this to ~3 fix attempts. If still failing, **halt and report** —
the default branch is never touched until verification is clean.
